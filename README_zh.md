...
* 关于系统级消息推送，目前作者有的三台移动设备测试完成，其他更多移动设备没有条件测试，如果消息推送无法收到，请检测首先是否允许访问外网，其次是否包含谷歌商店应用，这两者不是必须的，但是对于某些手机型号可能需要。如果无论如何也无法利用FCM发送消息，或者这种方式不能被业务接受，可以尝试使用移动设备所在的厂商提供的消息推送平台（鸿蒙推送、小米推送），或者其他第三方平台（JPush、Getui）
